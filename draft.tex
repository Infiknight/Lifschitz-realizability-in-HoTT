...
\begin{lem} 
The Law of Excluded Middle and Markov's Principle implies the Lesser Limited Principle of Omniscience.
\end{lem}
...
\end{multline*}
The fact that $\mathbb{N}$ is modal, follows from this in Intensional Type Theory. 
...
$\mathbb{N}$ is $\lVert B \rVert$-null.
\end{lem}
\begin{proof}
Given $a : A$ and $f : \lVert B \rVert \; a \rightarrow \mathbb{N}$ we need to prove that there exists unique $f' : \mathbf{1} \rightarrow \mathbb{N}$ through which $f$ factors, in the sense that $f = f' \circ g$, where $g : C\; a \rightarrow \mathbf{1}$ is the sole inhabitant of its function space. 
...
\bigg( \prod_{s : \mathbf{2}^\mathbb{N}} \Big(\big(\prod_{n : \mathbb{N}} s(n) = a(2 \cdot n)\big) \vee \big(\prod_{n : \mathbb{N}} s(n) = a(2\cdot n +1)\big) \Big) \rightarrow \\
	 \Big(\prod_{n : \mathbb{N}}s\; n = 0 \Big) \rightarrow \Big\lVert \sum_{z : \mathbb{N}} \prod_{p : B\; a} f^*\; p = z \Big\rVert \bigg)
\end{multline}
\begin{multline} \label{IPrhs}
\rightarrow \bigg( \prod_{s : \mathbf{2}^\mathbb{N}} \Big(\big(\prod_{n : \mathbb{N}} s(n) = a(2 \cdot n)\big) \vee \big(\prod_{n : \mathbb{N}} s(n) = a(2\cdot n +1)\big) \Big) \rightarrow \\
	  \Big\lVert \sum_{z : \mathbb{N}} \Big(\prod_{n : \mathbb{N}}s\; n = 0 \Big) \rightarrow \prod_{p : B\; a} f^*\; p = z \Big\rVert \bigg)
\end{multline}
where $f^*$ is the composition $f \circ C^B$, with $C^B$ being the canonical function $B\; a \rightarrow C\; a$. 
Note that the left hand side \ref{IPlhs} is provable. 
So we have a witness of the right hand side \ref{IPrhs}.
By providing the odd $s_1$ and even $s_2$ subsequences of $a$ to it, we get hold of $z_1 : \mathbb{N}$ and $z_2 : \mathbb{N} $ respectively. 
...
By function extensionality we reduce proving $f= f' \circ g$ to proving $f\; c = f'\circ g\; c$ for arbitrary $c : C\; a$. \\
...
\begin{lem}
If $C, D$ are $\lVert B \rVert$-null then so is $C + D$.
\end{lem}
\begin{proof}
...
Same as we did for \cref{NisBnull} we can lift a variant of IP, that will help us in our endeavor, from the model of Extensional Type Theory. 
This variant takes the following arguments in the form of a 4-product
\begin{align*}
&a: &A
\\
