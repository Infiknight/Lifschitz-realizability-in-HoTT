...
\title{Lifschitz Realizability for Homotopy Type Theory}%replace with the appropriate homework number
\author{Dimitrios Koutsoulis} %if necessary, replace with your course title
\birthdate{May 27, 1992}
\birthplace{Marousi, Attica, Greece}
\defensedate{November 26, 2019}
\supervisor{Andrew Swan}
\supervisor{Benno van den Berg}
\committeemember{Benno van den Berg (co-supervisor)}
\committeemember{Nick Bezhanishvili}
\committeemember{Jaap van Oosten}
\committeemember{Andrew Swan (supervisor)}
\committeemember{Yde Venema (chair)}
...
The first one starts with \Cref{ChapterTT} which provides a short introduction to a variant of Martin-L\"of's Type Theory. 
\Cref{ChapterModalities} introduces modalities and nullification which are the most important tools in our disposal. 
...
The second part is \Cref{RUSS}. 
This one can be further divided into two parts. 
The prologue of the chapter, up to and including \Cref{EPrime}, comprises an amendment of a fragment of \cite{1905.03014} and works towards procuring an intermediate model. 
...
%This section constitutes an informal treatment of Type Theory. 
...
Nevertheless, this is an informal presentation meant to reflect a formal system like the one in \Cref{formalTT}. 
...
This would be a good place to talk a bit more about the closure properties of judgmental/definitional equality $\equiv$. 
	...
	\end{rem}
	% With the above facts about judgmental equality in mind, along with the induction principle of equality, we can do the following in TT. 
	% We can concatenate those paths whose domains and codomains allow for it. 
	% That is, if $p : x = y$ and $p' : y = z$ are such paths then we can concatenate them $p \cdot p' : x = z$, the proof of this consists of invoking $\mathtt{ind}_{=}$ twice, once for $p$ and once for $p'$, then we have to prove $x=z$ where $x\equiv y$ and $y \equiv z$, a trivial result. 
	...
	% We conclude that paths form an equivalence relation. 
	...
	\end{proof}
	% \begin{lem}
	% For $A : \mathcal{U}$, $B : A \rightarrow \mathcal{U}$, $f : \prod_{a : A}B\; a$, $a_1, a_2 : A$ and $p : a_1 =_A a_2$, transporting $f\;a_1$ over $p$ results in an inhabitant of $B\; a_2$ which is equal to $f\; a_2$.\\
	% Formally
	% $$$$
	% \end{lem}
...
Then, by the equivalence mentioned in the fourth datum of \Cref{modality_definition} we have a quasi-inverse $r$ for the modal unit $\eta_{\eta^\bigcirc_A \circ f \;(z) = \mathtt{id}_{\bigcirc A} \; (z)}$, 
...
Propositional truncation is a modality as it possesses the required data outlined in \Cref{modality_definition}. 
...
By the fourth datum of \Cref{modality_definition} and the relevant induction principle, with implicit arguments $A$ and $x\mapsto \mathtt{ind}(g\circ \eta_A^\bigcirc )(x) = g(x) : \bigcirc A \rightarrow \mathcal{U}$, we can reduce it to constructing a witness for
...
\begin{proof}
By \Cref{modalUnitLeftInverse} it is enough to provide a left inverse for 
...
This equivalence will come in handy later in \Cref{nullTypesModel}, when we will inductively apply $\mathcal{L}_B$ to a type's construction hoping for the resulting type to be equivalent to the one we started with, or at least similar enough.
...
A cwf (category with families) model of type theory is a categorical construction which `realizes' all data of formal TT presented in \Cref{formalTT}. 
...
%\todo{should i argue more about this?} 
...
The proofs in this section are all about $\lVert B \rVert$-nullness i.e. equivalence between function types (see \Cref{Bnull}). 
...
For the unnamed third argument we work as in the proof of \Cref{NisBnull}. 
...
B\; a \arrow[rr, "\bar{f}"', bend right=49] \arrow[r, "|\cdot|"] & \lVert B\;a\rVert \arrow[r, "f"] \arrow[u, "h", dashed] \arrow[uu, "e", bend left=49] & C+D \arrow[r, "g"] & \mathbf{2}
\end{tikzcd}
\end{center}
...
\end{lem}
\begin{proof}
Observe that if $\mathbf{2} \simeq (\mathbf{1} + \mathbf{1})$ then the goal would follow from \Cref{coproductIsBnull} and the fact that $\mathbf{1}$ is trivially $\lVert B \rVert$-null. 
...
\end{lem}
\begin{proof}
Follows immediately from the fact that $\mathcal{L}_{\lVert B \rVert}$ is a modality and the fourth datum of \Cref{modality_definition}.
\end{proof}
...
\Cref{formalTT} provides a short description of a formal system in which the informal derivations we do in the rest of this text are meant to be taking place. 
...
\end{lem}
\begin{proof}
The types $\mathbb{N}$ and $\mathbf{2}$ are $\lVert B \rVert$-null in $\mathcal{E}'$ as shown in \Cref{NisBnull} and its corollary. 
It's trivial to show that the unit type $\mathbf{1}$ is $\lVert B \rVert$-null too. 
$\lVert B \rVert$-nullness of dependent products and sums under the assumptions above, follow from \Cref{reflSubuniversePiTypes} and the fact that nullification is a modality and therefore forms a $\sum$-closed reflective subuniverse as seen in \Cref{reflectiveSubuniverse}. 
$\lVert B \rVert$-nullness of identity types follows once again from the fact that nullification is a modality and the fourth datum of \Cref{modality_definition}. 
...
By \Cref{coproductIsBnull} we have that $\Gamma \vdash_{\mathcal{E}'_{\lVert B \rVert}} \llbracket C + D\rrbracket^{\mathcal{E}'} : \mathcal{U}$. 
...
The coproduct data of $ \llbracket C + D\rrbracket^{\mathcal{E}'}$ in $\mathcal{E}'$ are available in $\mathcal{E}'_{\lVert B \rVert}$, since their types must be $\lVert B \rVert$-null by \Cref{reflSubuniversePiTypes}. 
...
Note that $T$ and $U$ are both functions from $\mathbb{N}$ to $\mathbb{N}$, so the results in \Cref{wellPurported} extend to them. 
With this in mind, along with the general result of \Cref{wellPurported} and Corollary 5.6 of \cite{1905.03014}, we can deduce, by induction on type formation, that there is a path $p$ in $\mathcal{E}'_{\lVert B \rVert}$ between 
...
which we know to be inhabited by \Cref{EPrime}. 
...
where $\textnormal{LLPO} \equiv \prod_{a : A}\lVert B\;a \rVert$ for $A$ and $B$ presented in \Cref{AandB}. 
...
\end{proof}
With the above theorem in mind along with \Cref{ACCandCTandLLPOimplyBottom} we can deduce the following corollary. 
\begin{cor}
...
% Recall the full definition \Cref{LLPO} of LLPO
...
% So it is enough to prove $\lVert B\; a \rVert \rightarrow \mathcal{L}_{\lVert B \rVert}\;\lVert B\; a \rVert$, for which we already have a witness in the form of the first datum of Definition \Cref{modality_definition}, since $\mathcal{L}_{\lVert B \rVert}$ is a modality. 
...
All the constructions and rules described in \Cref{informalTT} should and can be transcribed in this form. 
Note that some parts of \Cref{informalTT} need not be formalized separately but can instead be special cases of one another. 
